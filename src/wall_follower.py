...
class WallFollower:
    ...
    def __init__(self):
        ...
        distance_k_d = rospy.get_param("wall_follower/distance_k_d", 2.0)
        self.distance_pid = PIDController(distance_k_p, distance_k_i, distance_k_d)
        self.angle_pid = PIDController(1.0, 0.0, 0.0)
    ...
    def extract_laser_data(self, scan):
        ...
        valid_angles = ((laser_data[1,
                         :]) * self.SIDE + self.extra_angle) >= 0  # positive angles if self.SIDE > 0 else negative angles
    ...
    def compute_heading(self, estimated_distance, estimated_wall_angle, front_dist):
        if front_dist < 2 * self.DESIRED_DISTANCE:
        ...
        angle_error = desired_angle - (-1 * estimated_wall_angle)
        desired_heading = self.angle_pid(angle_error)
    ...
    def generate_message(self, desired_heading):
        ...
        out_msg.drive.steering_angle_velocity = 0  # TODO: tune this
        out_msg.drive.speed = self.VELOCITY
        out_msg.drive.acceleration = 0  # TODO: tune this
        out_msg.drive.jerk = 0  # TODO: tune this
